    ...
    <div style={{ minHeight: '100vh', background: 'var(--mm-bg)' }}>
      ...
      <div className="mm-page-pad" style={{ maxWidth: 900, margin: '0 auto' }}>
        <div className="mm-content-detail" style={{ alignItems: 'start' }}>
          ...
          <div className={`mm-card mm-content-detail-thumb${content.thumbnail_url ? ' mm-content-detail-thumb--fit' : ''}`} style={{ overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--mm-primary-light)' }}>
            {content.thumbnail_url ? (
              // 依頼: 「サムネイルが拡大（トリミング）されて全体が分からない」。写真の縦横比なりに
              // 全体を見せる（.mm-content-detail-thumb--fit）。極端に縦長で高さを抑えたときの左右の
              // 余白には同じ画像をぼかして敷く（黒帯にしない）。
              <div style={{ position: 'relative', width: '100%', overflow: 'hidden' }}>
                ...
                <img src={content.thumbnail_url} alt={content.title} className="mm-content-detail-thumb-img" />
              </div>
